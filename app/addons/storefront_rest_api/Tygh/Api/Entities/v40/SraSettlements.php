...
class SraSettlements extends ASraEntity
{
    ...
    public function index($id = '', $params = [])
    {
        ...
        $order_info = [];
        if ($valid_params) {
            $order_info = fn_get_order_info($id);
            if (!$order_info || !$this->checkOrderOwner($order_info)) {
                $status = Response::STATUS_NOT_FOUND;
                $data['message'] = __('object_not_found', [
                    '[object]' => __('order')
                ]);
                $valid_params = false;
            }
    ...
    public function create($params)
    {
        ...
        if ($valid_params && isset($params['payment_method_id'])) {
            $configuration_provider = $this->getConfigurationProvider($payment_method);

            if ($configuration_provider) {
                $confirmation_result = new OperationResult(false);
                try {
                    $confirmation_result = $configuration_provider->confirmPayment($payment_method, $params['payment_method_id'], $order_info);
                } catch (Exception $e) {
                    $data['message'] = __('stripe.payment_intent_error', [
